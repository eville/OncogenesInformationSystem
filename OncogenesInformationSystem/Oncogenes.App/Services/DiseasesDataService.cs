        ...
        public async Task<Disease> GetDiseaseById(int id)
        {
            try
            {
                Disease? disease = await JsonSerializer.DeserializeAsync<Disease>
                          (await this.httpClient.GetStreamAsync($"api/Diseases/{id}"), new JsonSerializerOptions() { ReferenceHandler = ReferenceHandler.IgnoreCycles, PropertyNameCaseInsensitive = true });
                return disease;
            }
            catch (Exception exception)
            {
